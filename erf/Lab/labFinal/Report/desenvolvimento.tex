...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Imagens/cadence_LS_circuito_maxmat.png}
    \caption{Circuito adaptado com Linhas e Stubs ajustado para ganho e ruído.}
    \label{fig:cadence_LS_circuito_maxmat}
\end{figure}

As características de ganho e ruído do circuito adaptado podem ser vistas na figura \ref{fig:cadence_LS_values_maxmat}.
...
Apesar destes valores estarem de acordo com o esperado, é possível observar na figura \ref{fig:sparam_maxmat} um ligeiro desvio na frequência de operação ideal, estando desviada dos $6\,\text{GHz}$ em aproximadamente $0.2\,\text{MHz}$.
Este desvio pode ser atribuído a pequenas imprecisões na adaptação, visto que não foi realizada em computador, no entanto não deixa de estar fora do esperado, porque todas as adaptações anteriores foram feitas com o mesmo método, e não apresentaram este tipo de desvio.